...
My choice of representation is a integer array for SlotMachine2. I considered using an OSU Sequence data structure because it is similar to an array and allows for suplicates, but thought it would be better to implement as an array because the amount of slots is only going to be three and probably won't change. The fixed length of an array is helpful for this purpose. The representation will be restricted by the convention of the array size always being 3, never being empty/null, and always being a digit from 1-9. This is so that the slot machine always has 3 numbers and can print results. The representation will be interpreted by the slot machine results directly, which is the represented int array.
